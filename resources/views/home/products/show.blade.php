                ...
                <div class="col-lg-6 col-md-6 order-1 order-sm-1 order-md-2">
                    <div class="product-details-img">
                        <div class="zoompro-border zoompro-span">
                            <img class="zoompro" src="{{ generateProductImageLink($product->primary_image) }}"
                                data-zoom-image="{{ generateProductImageLink($product->primary_image) }}" alt="" />

                        </div>
                        <div id="gallery" class="mt-20 product-dec-slider">
                            <a data-image="{{ generateProductImageLink($product->primary_image) }}"
                                data-zoom-image="{{ generateProductImageLink($product->primary_image) }}">
                                <img width="90" src="{{ generateProductImageLink($product->primary_image) }}"
                                    alt="">
                            </a>
                            @foreach ($product->images as $image)
                                <a data-image="{{ generateProductImageLink($image->image) }}"
                                    data-zoom-image="{{ generateProductImageLink($image->image) }}">
                                    <img width="90" src="{{ generateProductImageLink($image->image) }}" alt="">
                                </a>
                            @endforeach
                        </div>
                    </div>
    ...
    <script>
        $('.variation-select').on('change', function() {
            ...
            if (variaction.is_sale) {
                let spanSale = $('<span/>', {
                    class: 'new',
                    text: toPersianNum(number_format(variaction.sale_price)) + ' تومان'
                });

                let spanPrice = $('<span/>', {
                    class: 'old',
                    text: toPersianNum(number_format(variaction.price)) + ' تومان'
                });
                variationsPriceDiv.append(spanSale);
                variationsPriceDiv.append(spanPrice);
            } else {
                let spanPrice = $('<span/>', {
                    class: 'new',
                    text: toPersianNum(number_format(variaction.price)) + ' تومان'
            ...
            $('.quantity-input').attr('data-max', variaction.quantity);
